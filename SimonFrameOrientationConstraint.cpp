...
void SimonFrameOrientationConstraint::initializeOnModelImpl(const Model& model, const MocoProblemInfo&) const
{
    ...
    //const double& minimum = -1.57;//-90*pi/180
...
void SimonFrameOrientationConstraint::calcPathConstraintErrorsImpl(const SimTK::State& state, SimTK::Vector& errors) const
{
    ...
    //auto& frame2 = getModel().getComponent<Frame>("/bodyset/torso");

    //SimTK::Vec3& frame1_pos = SimTK::Vec3(0, 0, 0);
    //auto& frame2_pos = frame2.getRotationInGround(state);

    //auto coordaxis = SimTK::CoordinateAxis(2);
    //auto angleVec = frame2_pos.convertOneAxisRotationToOneAngle(coordaxis);//convertRotationToAngleAxis();

    //auto relative_position = angleVec;//angleVec[0];

    ////std::cout << "the angle:" << relative_position << std::endl;
    ////system("pause");
    //auto projected = relative_position;//relative_position;
    //errors[0] = projected;
}
